...
import { linking } from 'repro-app/navigation/linking';
import type { NextNavigationProps } from 'repro-app/navigation/types';
import Router from 'next/router';
import { Platform } from 'react-native';
import { NextTabNavigator } from 'repro-app/navigation/next-tab-navigator';
...
function useLinkingConfig(trackedLinking: React.MutableRefObject<LinkingOptions<{}>>) {
  return {
...
export function Navigation({ Component, pageProps }: NextNavigationProps) {
  ...
  return (
    <NavigationContainer
      ...
      documentTitle={{
        enabled: true,
        formatter: (options) => (options?.title ? `${options.title} - Record Pool` : 'Record Pool'),
      }}
    >
      <LinkTo />
